...
#define FT_MOTION
  ...
  //#define FTM_SHAPER_E                        // ОТКЛЮЧЕНО — конфликтует с SPA (размазывает компенсацию)
...
//===========================================================================
//==================== SIMPLIFIED PRESSURE ADVANCE (SPA) v4.8 ===============
//===========================================================================
// SPA — Simplified Pressure Advance для Marlin FT Motion
// Пропорциональная модель offset = K × Ve + Slew Rate Limiter.
//
// Для активации необходимы ОБА макроса: SIMPLIFIED_PA и PA_LOOKAHEAD.
//
// v4.8: Добавлен Slew Rate Limiter (SPA_PA_MAX_RATE) для предотвращения
//        пропуска шагов экструдером при больших K на резких ускорениях.
//        В отличие от EMA-фильтра v4.5, лимитер не вносит фазовую задержку:
//        малые изменения Ve проходят без искажений, клиппируются только
//        аномальные пики Δoffset, превышающие физические возможности мотора.
// Подробнее: docs/SPA_Documentation.md
  ...
  #define PA_LOOKAHEAD                      // Включает основной алгоритм в calc_traj_point() и поля block_t
    ...
    // Task 5: Dynamic Volumetric Slew Rate Limiter (v4.9).
    // Основан на максимальном объёмном расходе хотенда, а не на произвольной
    // скорости изменения offset. SRL = V_f_max - |Ve|, где V_f_max = Q_max / A_fil.
    //
    // Асимметричный: разгон (Ve растёт) → жёсткий лимит (меньше Q остаётся),
    // торможение (Ve падает) → мягкий лимит (больше Q освобождается).
    //
    // Как подобрать:
    //   1) Измерьте максимальный объёмный расход хотенда для вашего
    //      филамента и температуры (калибровка MVS в OrcaSlicer).
    //   2) Установите PA_MAX_VOLFLOW = измеренное MVS [мм³/с].
    //   3) R0 = unlimited (только для диагностики, не рекомендуется).
    //
    // По умолчанию: 15 мм³/с (консервативно для PETG/PLA 0.4mm düse).
    ...
    // M900 R<value>   — SPA_PA_MAX_RATE (0 = unlimited, >0 = мм/с)
